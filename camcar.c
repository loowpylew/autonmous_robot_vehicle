...
void camcar(int argc, char *argv[], struct thread_dat *ptdat) 
{
    ...
    while (ch != 'q') {
        ...
        mvprintw(10, 1,"Status: blob(size=%d, halign=%d, blobnr=%u)  ", blob.size, blob.halign, ptdat->blobnr);
        ...
        if (obstacle) {
            ...
            // DONE: TODO: thread communication: copy blob from camera thread (mutex protected)
            blob = ptdat->blob;
		    ...
		    initio_SpinRight(100);
                    delay(200);  // TODO: check if reasonable
		    initio_Stop(0);
                    // TODO: potential actions: turn car or camera platform a few steps around and see if a blob is to be found
                    blobnr = ptdat->blobnr;
                }
            } else {
                carBlobAligned = (blob.halign >= -0.2 && blob.halign <= 0.2);  // TODO: adjust values to useful ones
